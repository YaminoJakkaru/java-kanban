public class Main {
    ...
    while (i<2){
        ...
        while (j<i){
            j++;
            Subtask subtask= new Subtask("Проверка субтаска "+i+j, " окей ", "NEW");
            manager.addSubtask(epic.getIdentificationNumber(),subtask);
        }
        ...
        Subtask subtask1= new Subtask("Проверка обновления субтаска", " окей ", "DONE");
        manager.updateSubtask(7,subtask1);
        manager.deleteById(6);
        Subtask subtask2= new Subtask("Проверка обновления субтаска 2", " окей ", "IN_PROGRESS");
